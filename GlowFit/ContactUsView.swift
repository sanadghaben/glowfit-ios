...
struct ContactUsView: View {
    ...
    var body: some View {
        AccountSheet(title: L("contact_us_title")) {
            ...
            // Divider label
            HStack { Rectangle().fill(Color.white.opacity(0.06)).frame(height: 1); Text(L("contact_or_send_message")).font(.custom("Tajawal-Regular",size:12)).foregroundColor(Color.white.opacity(0.3)).fixedSize(); Rectangle().fill(Color.white.opacity(0.06)).frame(height:1) }

            // Topic picker
            VStack(alignment: .leading, spacing: 8) {
                Text(L("contact_subject_label")).font(.custom("Tajawal-Medium",size:13)).foregroundColor(Color.white.opacity(0.5))
            ...
            // Subject
            EditField(label: L("contact_subject_field"), icon: "pencil", text: $subject)

            // Message
            VStack(alignment: .leading, spacing: 8) {
                Text(L("contact_details_label")).font(.custom("Tajawal-Medium",size:13)).foregroundColor(Color.white.opacity(0.5))
                ZStack(alignment:.topTrailing) {
                    ...
                    if message.isEmpty {
                        Text(L("contact_message_placeholder")).font(.custom("Tajawal-Regular",size:15)).foregroundColor(Color.white.opacity(0.2)).padding(22)
                    }
            ...
            Button(action: sendMessage) {
                HStack(spacing:10) {
                    if isSending {
                        ...
                        Text(L("contact_send_message")).font(.custom("Tajawal-Bold",size:17))
                    }
                }
                .foregroundColor(.white)
            ...
            HStack(spacing:6) {
                ...
                Text(L("contact_reply_time")).font(.custom("Tajawal-Regular",size:12)).foregroundColor(Color.white.opacity(0.4))
            }
        ...
        .alert("تم الإرسال ✅", isPresented: $showSuccessAlert) {
            ...
            Text(L("contact_success_message"))
        }
    }
